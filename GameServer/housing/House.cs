...
namespace DOL.GS.Housing
{
    public class House
    {
        ...
        {
            get => Position.Create(_databaseItem.RegionID, _databaseItem.X, _databaseItem.Y, _databaseItem.Z, (ushort)(_databaseItem.Heading/180.0*2048));
            set
            {
                _databaseItem.X = value.X;
                _databaseItem.Y = value.Y;
                _databaseItem.Z = value.Z;
                _databaseItem.Heading = value.Orientation.InDegrees;
                _databaseItem.RegionID = value.RegionID;
            }
        }
        ...
        {
            get => (ushort)_databaseItem.Heading;
            ...
            if (addToInventory)
            {
                ...
                if (template != null)
                {
                    ...
                    if (player.Inventory.AddItem(eInventorySlot.FirstEmptyBackpack, item))
                        InventoryLogging.LogInventoryAction(player.CurrentHouse.DatabaseItem.ObjectId, "(HOUSE;" + HouseNumber + ")", player, eInventoryActionType.Loot, item, item.Count);
                }
            }
        }
        ...
        public void Edit(GamePlayer player, List<int> changes)
        {
            ...
            {
                InventoryLogging.LogInventoryAction(player, player.CurrentHouse.DatabaseItem.ObjectId, $"(HOUSE;{HouseNumber})", eInventoryActionType.Merchant, price);
                return;
            }
