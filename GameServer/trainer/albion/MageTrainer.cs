    ...
    public class MageTrainer : GameTrainer
    {
        ...
        public override bool Interact(GamePlayer player)
        {
            ...
            if (player.CharacterClass.ID == (int)TrainedClass)
            {
                ...
                if (player.Level >= 5)
                {
                    player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "MageTrainer.Interact.Text1", this.Name), eChatType.CT_Say, eChatLoc.CL_PopupWindow);
                }
                else
                {
                ...
                {
                    player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "MageTrainer.Interact.Text2", this.Name), eChatType.CT_Say, eChatLoc.CL_PopupWindow);
                }
            }
            else
            {
        ...
        public override bool WhisperReceive(GameLiving source, string text)
        {
            ...
            switch (text)
            {
                ...
                case "Cabaliste":
                    ...
                    {
                        player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "MageTrainer.Cabalist.Refuse", this.Name), eChatType.CT_Say, eChatLoc.CL_ChatWindow);
                    }
                    return true;
                case "Sorcerer":
                case "Sorcier":
                    ...
                    {
                        player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "MageTrainer.Sorcerer.Refuse", this.Name), eChatType.CT_Say, eChatLoc.CL_ChatWindow);
                    }
                    return true;
                case "practice staff":
