...
namespace DOL.Geometry
{
      ...
      : IComparable, IComparable<Vector3>, IEquatable<Vector3>, IFormattable
    {
        ...
        /// <param name="p2"></param>
        /// <returns></returns>
        public ushort GetHeadingToLocation(Vector3 p2)
        {
            var dx = (long)p2.X - X;
            var dy = (long)p2.Y - Y;
            var heading = (ushort)(Math.Atan2(-dx, dy) * Point2D.RADIAN_TO_HEADING);
            if (heading < 0)
            {
                heading += 0x1000;
            }
        ...
        /// <summary>
        ///   Find the distance between two Vectors but no Sqrt on it
        ...
        public static double DistanceSquared(Vector3 v1, Vector3 v2)
        {
            ...
            return
              v1.Z != 0 && v2.Z != 0
                ? // ignore Z if one is 0
                dx * dx + dy * dy + (v1.Z - v2.Z) * (v1.Z - v2.Z)
                : dx * dx + dy * dy;
        }

        /// <summary>
        ///   Find the distance between two Vectors ignoring Z
        ///   Pythagoras theorem on two Vectors
        /// </summary>
        /// <param name="v1">The Vector3 to find the distance from </param>
        /// <param name="v2">The Vector3 to find the distance to </param>
        ...
        ///   Pythagoras theorem on two Vectors but no qert on it
        /// </summary>
        /// <param name="other">The Vector3 to find the distance to </param>
        /// <returns>The distance between two Vectors</returns>
        /// <implementation>
        ///   <see cref="DistanceSquared(Vector3, Vector3)" />
        ///   Overload for DistanceSquared method, finds squared distance between this Vector3 and another
        ///   Uses the DistanceSquared(Vector3,Vector3) method to avoid code duplication
        /// </implementation>
        ...
        {
            var x = v1.Z * Math.Sin(degree) + v1.X * Math.Cos(degree);
            var y = v1.Y;
            var z = v1.Z * Math.Cos(degree) - v1.X * Math.Sin(degree);
        ...
        {
            var x = v1.X;
            var y = v1.Y * Math.Cos(degree) - v1.Z * Math.Sin(degree);
            var z = v1.Y * Math.Sin(degree) + v1.Z * Math.Cos(degree);
            return new Vector3(x, y, z);
        }
        ...
        ///   Important: In DAoC coord system this will rotate clockwise!
        /// </summary>
        /// <param name="v1">The Vector3 to be rotated</param>
        /// <param name="degree">The angle to rotate the Vector3 around in degrees</param>
        /// <returns>Vector3 representing the rotation around the Z axis</returns>
        public static Vector3 RotateOnZ(Vector3 v1, double degree)
        {
            var radian = degree * DegreeToRadian;
            var cos = Math.Cos(radian);
            var sin = Math.Sin(radian);
            var x = v1.X * cos - v1.Y * sin;
            var y = v1.X * sin + v1.Y * cos;
            var z = v1.Z;
            return new Vector3(x, y, z);
        }
        ...
        /// </implementation>
        public void RotateOnZ(double degree)
        {
            this = RotateOnZ(this, degree);
        }

        public Vector3 Offset(int dx, int dy, int dz)
        {
        ...
        */
        ...
        public string ToVerbString()
        {
            ...
            output += string.Format("( x={0}, y={1}, z={2} )", (int)X, (int)Y, (int)Z);
        ...
        /// <summary>
        ///   Check distance to other point
        ///   If in range it gives true
        ///   A bit better performance than calculating distance as it saves the Sqrt call
        /// </summary>
        /// <param name="pos"></param>
        /// <param name="range"></param>
        /// <returns></returns>
        public bool IsInRange(Vector3 pos, int range)
        {
            var squaredRange = (double)range * range;
            ...
            var dz = Z - pos.Z;
            return dx * dx + dy * dy + dz * dz <= squaredRange;
        }

        /// <summary>
        ///   Check distance to other point, no Z involved
        ///   If in range it gives true
        ///   A bit better performance than calculating distance as it saves the Sqrt call
        /// </summary>
        /// <param name="pos"></param>
        /// <param name="range"></param>
        /// <returns></returns>
        public bool IsInRange2D(Vector3 pos, int range)
        {
            var dx = X - pos.X;
            var dy = Y - pos.Y;
        ...
        public const double DegreeToRadian = 2 * Math.PI / 360;
        public const double RadianToDegree = 1 / DegreeToRadian;
