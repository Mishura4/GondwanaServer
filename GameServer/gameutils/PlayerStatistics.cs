...
namespace DOL.GS
{
    public class PlayerStatistics : IPlayerStatistics
    {
        ...
        public static void CreateServerStats(GameClient client)
        {
            ...
            foreach (DOLCharacters chr in chars)
            {
                if (chr.IgnoreStatistics == false)
                {
                    ...
                    {
                        toplist.Add("\n" + LanguageMgr.GetTranslation(client, "GameUtils.Player.Statistics.Top20GeneralEntry", count.ToString(), chr.Name, string.Format("{0:0,0}", chr.RealmPoints), (chr.RealmLevel + 10) / 10, (chr.RealmLevel + 10) % 10));
                        if (++count > 20)
                            break;
                    }
                }
            ...
            if (count == 1)
            {
                toplist.Add(LanguageMgr.GetTranslation(client, "GameUtils.Player.Statistics.NoneFound"));
            }
            #endregion /stats top
            ...
            for (int c = 0; c < allstatsrp.Count; c++) { if (c > 19 || allstatsrp[c].count < 1) break; statsrp += LanguageMgr.GetTranslation(client, "GameUtils.Player.Statistics.Top20RPEntry", c + 1, allstatsrp[c].name, allstatsrp[c].count.ToString()) + "\n"; }
            for (int c = 0; c < allstatslrp.Count; c++) { if (c > 19 || allstatslrp[c].count < 1) break; statslrp += LanguageMgr.GetTranslation(client, "GameUtils.Player.Statistics.Top20RPHourEntry", c + 1, allstatslrp[c].name, allstatslrp[c].count.ToString()) + "\n"; }
            for (int c = 0; c < allstatskills.Count; c++) { if (c > 19 || allstatskills[c].count < 1) break; statskills += LanguageMgr.GetTranslation(client, "GameUtils.Player.Statistics.Top20KillersEntry", c + 1, allstatskills[c].name, allstatskills[c].count.ToString()) + "\n"; }
            for (int c = 0; c < allstatsdeath.Count; c++) { if (c > 19 || allstatsdeath[c].count < 1) break; statsdeath += LanguageMgr.GetTranslation(client, "GameUtils.Player.Statistics.Top20DeathblowsEntry", c + 1, allstatsdeath[c].name, allstatsdeath[c].count.ToString()) + "\n"; }
            for (int c = 0; c < allstatsirs.Count; c++) { if (c > 19 || allstatsirs[c].count < 1) break; statsirs += LanguageMgr.GetTranslation(client, "GameUtils.Player.Statistics.Top20IRSEntry", c + 1, allstatsirs[c].name, allstatsirs[c].count.ToString()) + "\n"; }
            for (int c = 0; c < allstatsheal.Count; c++) { if (c > 19 || allstatsheal[c].count < 1) break; statsheal += LanguageMgr.GetTranslation(client, "GameUtils.Player.Statistics.Top20HealersEntry", c + 1, allstatsheal[c].name, allstatsheal[c].count.ToString(), allstatsrpearnedfromheal[c].count.ToString()) + "\n"; }
            for (int c = 0; c < allstatsres.Count; c++) { if (c > 19 || allstatsres[c].count < 1) break; statsres += LanguageMgr.GetTranslation(client, "GameUtils.Player.Statistics.Top20ResurrectorsEntry", c + 1, allstatsres[c].name, allstatsres[c].count.ToString()) + "\n"; }
        ...
        public virtual string GetStatisticsMessage()
        {
            ...
            string stringOnlineTime = LanguageMgr.GetTranslation(Player.Client, "GameUtils.Player.Statistics.OnlineTimeDays", onlineTime.Days, onlineTime.Hours, onlineTime.Minutes, onlineTime.Seconds) + "\n";
            if (onlineTime.Days < 1)
                stringOnlineTime = LanguageMgr.GetTranslation(Player.Client, "GameUtils.Player.Statistics.OnlineTime", onlineTime.Hours, onlineTime.Minutes, onlineTime.Seconds) + "\n";

            // First line should include all the options available for the stats command
            string message = LanguageMgr.GetTranslation(Player.Client, "GameUtils.Player.Statistics.Options") + "\n" +
                             LanguageMgr.GetTranslation(Player.Client, "GameUtils.Player.Statistics.Header", Player.Name) + "\n" +
                             LanguageMgr.GetTranslation(Player.Client, "GameUtils.Player.Statistics.TotalRP", TotalRP) + "\n" +
                             LanguageMgr.GetTranslation(Player.Client, "GameUtils.Player.Statistics.RealmPointsEarnedFromKills", RealmPointsEarnedFromKills) + "\n" +
                             LanguageMgr.GetTranslation(Player.Client, "GameUtils.Player.Statistics.KillsThatHaveEarnedRPs", KillsThatHaveEarnedRPs) + "\n" +
                             LanguageMgr.GetTranslation(Player.Client, "GameUtils.Player.Statistics.Deathblows", Deathblows) + "\n" +
                             LanguageMgr.GetTranslation(Player.Client, "GameUtils.Player.Statistics.Deaths", Deaths) + "\n" +
                             LanguageMgr.GetTranslation(Player.Client, "GameUtils.Player.Statistics.Healed", HitPointsHealed, RPEarnedFromHitPointsHealed) + "\n" +
                             LanguageMgr.GetTranslation(Player.Client, "GameUtils.Player.Statistics.RessurectionsPerformed", RessurectionsPerformed) + "\n" +
                             stringOnlineTime +
                             LanguageMgr.GetTranslation(Player.Client, "GameUtils.Player.Statistics.RPperHour", RPsPerHour(TotalRP, onlineTime)) + "\n" +
                             LanguageMgr.GetTranslation(Player.Client, "GameUtils.Player.Statistics.KillsperDeath", Divide((uint)KillsThatHaveEarnedRPs, (uint)Deaths)) + "\n" +
                             LanguageMgr.GetTranslation(Player.Client, "GameUtils.Player.Statistics.RPperKill", Divide(RealmPointsEarnedFromKills, (uint)KillsThatHaveEarnedRPs)) + "\n" +
                             LanguageMgr.GetTranslation(Player.Client, "GameUtils.Player.Statistics.RemainStanding", Divide(RealmPointsEarnedFromKills, (uint)Deaths)) + "\n";
            return message;
        }
        ...
        public virtual void DisplayServerStatistics(GameClient client, string command, string playerName)
        {
            ...
            {
                case "top":
                    client.Out.SendCustomTextWindow(LanguageMgr.GetTranslation(client, "GameUtils.Player.Statistics.Top20Players"), toplist);
                    break;
                case "rp":
                    client.Player.Out.SendMessage(LanguageMgr.GetTranslation(client, "GameUtils.Player.Statistics.Top20RP") + "\n" + statsrp, eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    break;
                case "lrp":
                    client.Player.Out.SendMessage(LanguageMgr.GetTranslation(client, "GameUtils.Player.Statistics.Top20RPHour") + "\n" + statslrp, eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    break;
                case "kills":
                    client.Player.Out.SendMessage(LanguageMgr.GetTranslation(client, "GameUtils.Player.Statistics.Top20Killers") + "\n" + statskills, eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    break;
                case "deathblows":
                    client.Player.Out.SendMessage(LanguageMgr.GetTranslation(client, "GameUtils.Player.Statistics.Top20Deathblows") + "\n" + statsdeath, eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    break;
                case "irs":
                    client.Player.Out.SendMessage(LanguageMgr.GetTranslation(client, "GameUtils.Player.Statistics.Top20IRS") + "\n" + statsirs, eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    break;
                case "heal":
                    client.Player.Out.SendMessage(LanguageMgr.GetTranslation(client, "GameUtils.Player.Statistics.Top20Healers") + "\n" + statsheal, eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    break;
                case "rez":
                    client.Player.Out.SendMessage(LanguageMgr.GetTranslation(client, "GameUtils.Player.Statistics.Top20Resurrectors") + "\n" + statsres, eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    break;
                case "player":
                    GameClient clientc = WorldMgr.GetClientByPlayerName(playerName, false, true);
                    if (clientc == null)
                    {
                        client.Player.Out.SendMessage(LanguageMgr.GetTranslation(client, "GameUtils.Player.Statistics.NoPlayer", playerName), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        return;
                    }

                    if (clientc.Player.StatsAnonFlag)
                    {
                        client.Player.Out.SendMessage(LanguageMgr.GetTranslation(client, "GameUtils.Player.Statistics.Statsanon", playerName), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                        return;
                    }
                ...
                default:
                    client.Player.Out.SendMessage(LanguageMgr.GetTranslation(client, "GameUtils.Player.Statistics.Options"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                    break;
            }
