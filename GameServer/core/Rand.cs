...
namespace DOL.Numbers
{
    ...
    public static class Rand
    {
        ...
        {
            get { return _rng ?? (_rng = new Random((int)DateTime.UtcNow.Ticks)); }
        }
        ...
        public static T ChooseOne<T>(ICollection<T> list)
        {
            ...
            {
                log.Error($"Exception caught while retrieving random element from list. List count: {count}, chosen index: {randomIndex}");
                return list.FirstOrDefault();
            }
        }
