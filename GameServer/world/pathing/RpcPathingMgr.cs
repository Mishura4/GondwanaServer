    ...
    public class RpcPathingMgr : IPathingMgr
    {
        ...
        public bool Init()
        {
            ...
            try
            {
                ...
                var response = client.ListLoadedNavmeshes(new ListLoadedNavmeshesRequest(), new CallOptions(deadline: DateTime.UtcNow.AddSeconds(15)));
                foreach (var mesh in response.Navmeshes)
                {
                    NavmeshAvailable[mesh] = true;
                    var zone = WorldMgr.GetZone((ushort)mesh);
                    if (zone != null)
        ...
        public async Task<WrappedPathingResult> GetPathStraightAsync(Zone zone, Vector3 start, Vector3 end)
        {
            ...
            return new WrappedPathingResult
            {
                ...
                Points = response.Path
                    .Select(x => new WrappedPathPoint { Flags = (dtPolyFlags)x.Flags, Position = ToVector3(x.Position) })
                    .ToArray()
            };
        }
        ...
        {
            return zone != null && NavmeshAvailable[zone.ID];
        }
